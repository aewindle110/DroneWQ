...
where $L_W$ ($W\,m^{-2}\,nm^{-1}\,sr^{-1}$) is water-leaving radiance, $E_d$ ($W\,m^{-2}\,nm^{-1}$) is downwelling irradiance, $\theta$ represents the sensor viewing angle between the sun and the vertical (zenith), $\phi$ represents the angular direction relative to the sun (azimuth) and λ represents wavelength. 
