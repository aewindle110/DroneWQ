...
import dronewq
from dronewq import Dls_ed, Mobley_rho, settings

test_path = Path(__file__).absolute().parent
test_path = test_path.joinpath("test_set")

settings.configure(main_dir=str(test_path))
...
pipeline = dronewq.RRS_Pipeline(
    main_dir=str(test_path),
    lw_method=Mobley_rho(),
    ed_method=Dls_ed(str(test_path)),
)
...
def test_Rrs():
    ...
    rrs_imgs = dronewq.load_imgs(str(pipeline.rrs_dir))
    results = []
