...
from dronewq.lw_methods.blackpixel import __compute
...
class TestComputeFunction:
    ...
    def test_compute_output_has_correct_bands(
        self, single_test_image, lsky_median, output_dir
    ):
    ...
    def test_compute_preserves_spatial_shape(
        self, single_test_image, lsky_median, output_dir
    ):
    ...
    def test_compute_physical_equation_holds(
        self, single_test_image, lsky_median, output_dir
    ):
        ...
        with (
            rasterio.open(single_test_image) as lt_src,
            rasterio.open(output_file) as lw_src,
        ):
            lt = lt_src.read()
            lw = lw_src.read()
