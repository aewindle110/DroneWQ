...
def write_exif_csv(img_set, outputPath):
    """
    This function saves specific EXIF metadata as a .csv. More metadata could be added based on what is needed in your workflow.
    """
...
def save_images(img_set, outputPath, thumbnailPath, warp_img_capture, generateThumbnails = True, overwrite=False):
    """
    This function does the actual processing of running through each capture within an imageset, computing radiance, aligning via a warp matrix, and saving as a .tif and optional .jpg.
    """
...
def process_micasense_subset(project_dir, warp_img_dir=None, overwrite=False, sky=False):
    """
    This function takes in an image directory and the applies save_images() function to save warped, cropped images with units of radiance.
...
def std_glint_removal_method(lt_dir, glint_corrected_lt_dir, glint_std_factor=1):
    """
    the std_glint_removal_method() filters NIR pixels > than mean+std*glint_std_factor. The lower it is, the more pixels will be masked. Same pixel indicies are masked across all bands. 
...
def mobley_rho_method(sky_lt_dir, lt_dir, lw_dir, rho = 0.028): 
    """
    The mobley_rho_method() uses a single (or small set of) Lsky image(s) and rho value of 0.028 to calculate Lw for each image. This approach should only be used if sky conditions are not changing substantially during the flight. 
...
def blackpixel_method(sky_lt_dir, lt_dir, lw_dir):
    """
    The blackpixel_method() uses the black (or dark) pixel assumption (NIR=0) to derive rho which is used to calculate rrs across all bands
...
def panel_ed(panel_dir, lw_dir, rrs_dir, output_csv_path):
    """
    This function calculates Ed from the panel and then divides the Lw images to calculate Rrs.
    """
...
def dls_ed(raw_water_dir, lw_dir, rrs_dir, output_csv_path):
    """
    This function calculates Ed from the downwelling light sensor (DLS) and then divides the Lw images to calculate Rrs
    """
...
def process_raw_to_rrs(main_dir, output_csv_path, glint_correct=True, glint_std_factor=2, lw_method='mobley_rho_method', ed_method='panel'):
    """
    This is the main processing script that processs raw imagery to units of remote sensing reflectance. Arguments are:
