...
def process_raw_to_rrs(main_dir, rrs_dir_name, output_csv_path, lw_method='mobley_rho_method', random_n=10, mask_pixels=False, pixel_masking_method='value_threshold', mask_std_factor=1, nir_threshold=0.01, green_threshold=0.005, ed_method='dls_ed', dls_corr=False, overwrite=False, clean_intermediates=True):
    """
    ...
    dls_corr: This option corrects the DLS method of measuring Ed with the panel value and then applies that correction across all images. Default is False. Only need if ed_method='dls_ed'.
    overwrite:
