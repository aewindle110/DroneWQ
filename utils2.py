...
# glint removal
def rrs_pixel_masking(main_dir, rrs_dir, output_dir_name, nir_threshold = 0.01, green_threshold = 0.005):
    """
    ...
    masked_rrs_dir = main_dir + '/' + output_dir_name
    ...
    for im in glob.glob(rrs_dir + "/*.tif"):
        with rasterio.open(im, 'r') as rrs_src:
            ...
            with rasterio.open(os.path.join(main_dir, masked_rrs_dir, im_name), 'w', **profile) as dst:
    ...
    fullCsvPath = write_metadata_csv(img_set, masked_rrs_dir)
...
def process_raw_to_rrs(main_dir, rrs_dir_name, output_csv_path, lw_method='mobley_rho_method', random_n=10, clean_intermediates=True, glint_correct=False, glint_std_factor=1, ed_method='dls_ed', overwrite=False):
    """
    ...
    if glint_correct == True:
        Path(glint_corrected_lt_dir).mkdir(parents=True, exist_ok=True)
...
def georeference(main_dir, rrs_dir, output_dir_name, flip=False, plot=False):
    """
    ...
    rrs_imgs, rrs_img_metadata = retrieve_imgs_and_metadata(img_dir = rrs_dir)

    for i in range(len(rrs_img_metadata)):
        ...
        with rasterio.Env():
            ...
            with rasterio.open(os.path.join(rrs_dir, 'capture_' + str(i+1) + '.tif'), 'r') as src:
                profile = src.profile
