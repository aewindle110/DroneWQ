...
def nir_correction(sky_lt_dir, lt_dir, lw_dir):
...
def hochberg_deglinting(lt_dir, lw_dir):
    for im in glob.glob(lt_dir + "/*.tif"):
        with rasterio.open(im, 'r') as lt_src:
            profile = lt_src.profile
            lt_all = []
            ...
            min_lt_NIR = np.percentile(stacked_lt_reshape[4,:], .1) #calculate minimum 10% of Ruas(NIR)

            #apply linear regression between NIR and visible bands 
            all_slopes = []
            for j in range(1,6):
                slopes = np.polyfit(stacked_lt_reshape[4,:], stacked_lt_reshape[j,:], 1)[0]
                all_slopes.append(slopes)

            #calculate Rrs (Ruas - b(Ruas(NIR)-min(Ruas(NIR))))
            Lw_all = []
            for j in range(1,6):
                Lw = stacked_Lt[j,:,:] - all_slopes[j]*(stacked_Lt[4,:,:]-min_lt_NIR)
                Lw_all.append(Lw)
            stacked_Lw = np.stack(Lw_all) #stack into np.array
...
def dls_Ed(raw_water_dir, output_dir):
    ...
    Ed = pd.DataFrame.from_records(Ed_data, index='image', columns = Ed_columns)
    Ed.to_csv(output_dir+'/DLS_Ed.csv')
...
def process_raw_to_rrs(main_dir, ed_method='panel', glint_correct=True, glint_std_factor=2, sky_reflection_correction='nir_baseline'):
    ...
    #elif sky_reflection_correction == 'deglinting':
      #  ruas_dir = ?
