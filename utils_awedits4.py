...
def retrieve_imgs_and_metadata(img_dir, count=10000, start=0, altitude_cutoff = 0):
    ...
    all_imgs = []
    for im in glob.glob(img_dir+'/*.tif'):
...
def process_raw_to_rrs(main_dir, output_csv_path, lw_method='mobley_rho_method', ed_method='dls_ed'):
    """
    ...
    process_micasense_images(main_dir, warp_img_dir=main_dir+'/align_img', overwrite=True, sky=False)
    ...
    if lw_method in ['mobley_rho_method','blackpixel_method']:
        ...
        process_micasense_images(main_dir, warp_img_dir=None, overwrite=True, sky=True)
    ...
    if  lw_method == 'mobley_rho_method':
        mobley_rho_method(sky_lt_dir, lw_dir)
...
def chl_gitelson(Rrsred, Rrsrededge):
    """
    This algortithm estimates chlorophyll a concentrations using the Gitelson et al. (___) algorithm. This algorithm is recommended for coastal (Case 2) waters.
    """
